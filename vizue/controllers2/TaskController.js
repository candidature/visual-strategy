...
export const getSingleTask = async (req, res) => {
    ...
    console.log("now inside get single story: " , initiative +" taskId "+ taskId )
    ...
    const initiativeFromDb = await FlowSchema
        ...
        .then(async (document) => {
            let foundNode;
            if (!document) {
                return res.status(StatusCodes.NOT_FOUND).json({"message": "task is empty for this initiative or no such task"});
                ...
                if(Object.keys(document).length !== 0) {
                    ...
                    return res.status(StatusCodes.NOT_FOUND).json({"message": "No task found under initiative "+ initiative});
                }
...
export const updateNewTask = async (req, res) => {
    const { initiative } = req.params;
    console.log("inside new update task")
...
//Delete initiative
export const deleteNewTask = async (req, res) => {
    const { initiative } = req.params;
    ...
    .then(document=>{
        if(document) {
            ...
            if(Object.keys(document).length !== 0) {
                ...
                document?.nodes.map((node)=>{
                    if (node?.id === req.body?.nodes?.id && node.type==="TASK_NODE") {
...
export const createNewTask = async (req, res) => {
    ...
    //    await FlowSchema.findOne({$and:[{"nodes.type": "INITIATIVE_NODE"},{"nodes.data.name":initiative}, {"nodes.data.state":{ "$ne": 'ARCHIVED' }}]})

    // await FlowSchema.findOne({
    //     $and:[
    //         {"nodes.type": "INITIATIVE_NODE"},
    //         {"nodes.data.name":initiative}, 
    //         {"nodes.data.state":{ "$ne": "ARCHIVED" }}
    //     ]})
