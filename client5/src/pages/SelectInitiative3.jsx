...
export default function SelectInitiative3() {
    ...
    const handleBack = () => {
        setActiveStep((prevActiveStep) => {
          if(prevActiveStep=>0 && prevActiveStep <= fullHistory.data.length-1){
            ...
            setData({data: fullHistory.data[prevActiveStep-1]})
            return (prevActiveStep - 1)
          } else {
            toast.error("Reached at history "+prevActiveStep);
          }
    ...
    function handleOnChange(event){
      ...
      setCurrentInitiative(splitOutput[0])
    ...
    return (
        ...
        {fullHistory.data?.length < 1 ? "No history":
            <MobileStepper
          variant="dots"
          steps={fullHistory.data?.length+1}
          position="static"
          activeStep={activeStep}
          sx={{ minWidth: 300, maxWidth: 1000, flexGrow: 50 }}
          nextButton={
            <Button size="small" onClick={handleNext} disabled={activeStep >= fullHistory.data?.length}>
              Next
              {theme.direction === 'rtl' ? (
                <KeyboardArrowLeft />
              ) : (
...
SelectInitiative3.loader = loader;